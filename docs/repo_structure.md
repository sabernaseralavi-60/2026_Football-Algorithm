...
> **می‌خوای فقط ادامه بدی، نه بفهمی چیه؟** برو مستقیم سراغ `docs/NEXT_STEPS.md` — اون سند
> دقیقاً می‌گه الان کجای کاریم و قدم بعدی چیه (با دستورهای اجرایی آماده)، طوری که کافیه بگی
> «طبق `docs/NEXT_STEPS.md` ادامه بده» و از همون‌جا شروع بشه.
...
- `specs/001-football-algorithm/tasks.md` الان به‌روزه: چک‌باکس‌های T001–T116 تیک خوردن
  (Phase 1–3 کامل). قبل از هر کار جدید همون رو چک کن ببین کجای ۱۶۰ تسک هستیم.
- **برای دستور اجرایی دقیقِ «قدم بعدی چیه»، `docs/NEXT_STEPS.md` رو بخون** — اون سند از این
  فایل به‌روزتره و شامل وضعیت واقعیِ Phase 4 (که ناتموم مونده) و دستورهای resume‌شونه.
- گزارش پایلوت آزمایشی (کوچک، فقط برای اطمینان از مسیر درست، نه نتیجهٔ نهایی) در `results/raw/pilot/pilot_report.json` هست. جدا از اون، `results/raw/main/` الان یه دیتاست جزئیِ واقعی داره (نه پایلوت) — جزئیاتش توی `NEXT_STEPS.md`.
