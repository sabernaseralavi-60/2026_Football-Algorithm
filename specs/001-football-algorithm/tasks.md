...
- [x] T001 Create the source-tree skeleton per plan.md's Project Structure: `src/tfo/`,
  ...
  package
- [x] T002 [P] Create `pyproject.toml` declaring the `tfo` and `tfo_bench` packages (src-layout),
...
- [x] T003 [P] Create `requirements-lock.txt` pinning exact versions: `cec2017-py` at git commit
...
- [x] T004 [P] Create `requirements-nodeps.txt` pinning `mealpy==3.0.3`, installed with
...
- [x] T005 [P] Update `.gitignore`: ignore `results/_smoke/`, `.venv/`, `__pycache__/`, build
  artifacts; keep `results/raw/`, `results/audit/`, `results/derived/`, `results/manifests/` tracked
- [x] T006 Vendor the sibling's `algorithms.py` at commit `96af96bbb5ef36514ae11a0cc5695d4a6211d9a4`
...
- [x] T007 [P] Write `src/tfo_bench/algorithms/vendor/PROVENANCE.md`: source URL, commit SHA
...
- [x] T008 [P] Create placeholder `config/protocol.toml`, `config/tuning_set.toml`,
...
- [x] T009 [P] Unit test `tests/unit/test_config.py`: `TFOConfig` loads from TOML, `config_hash` is
...
- [x] T010 [P] Unit test `tests/unit/test_squad.py`: property tests for data-model.md A1's validation
...
- [x] T011 [P] Unit test `tests/unit/test_clock.py`: "Fixture k ends at the first iteration end with
...
- [x] T012 [P] Unit test `tests/unit/test_seeds.py`: per-mechanism RNG streams spawned from
...
- [x] T013 [P] Unit test `tests/unit/test_account.py` (property-based, `hypothesis`): ledger
...
- [x] T014 [P] Implement `src/tfo/registry.py`: the mechanism tag ↔ module ↔ family ↔ citation
...
- [x] T015 [P] Implement `src/tfo/config.py`: the `TFOConfig` frozen dataclass with the `enable`
...
- [x] T016 [P] Implement `src/tfo/squad.py`: Squad structure-of-arrays (`X, f, P, f_P, slot, stamina,
  ...
  (makes T010 pass)
- [x] T017 [P] Implement `src/tfo/clock.py`: `MatchClock` (`B, n_fixtures, fixture, iteration, t`)
  and the ordered fixture-boundary hooks (makes T011 pass)
- [x] T018 [P] Implement `src/tfo/rng.py`: per-mechanism RNG stream spawning from `SeedSequence` at
  fixed registry indices (makes T012 pass)
- [x] T019 Implement `src/tfo/account.py`: `EvalAccount` with `evaluate(X, tag)` truncation-at-budget,
...
- [x] T020 [P] Implement `src/tfo/neutral.py`: the neutral (1+1)-ES fallback move
  `y = x_i + σ·s_i·N(0, I)`, tag `neutral`, greedy acceptance (research.md R14)

### 2.2 Player archetypes (8 mechanisms, FR-005–FR-012)

- [x] T021 [P] Unit test `tests/unit/test_sweeper_keeper.py`: "`A[0]` always equals the incumbent;
  the separation between members is at least `min_sep`; the archive never exceeds K members"
- [x] T022 [P] Unit test `tests/unit/test_zonal_centre_back.py`: "every proposal lies inside the
  agent's stratum; after a redraw, the strata are disjoint and their projections tile [0, 1]"
- [x] T023 [P] Unit test `tests/unit/test_overlapping_wing_back.py`: "y lies coordinate-wise between
  c and o; the keep-better rule holds"
- [x] T024 [P] Unit test `tests/unit/test_deep_lying_playmaker.py`: "the partner's graph distance is
  at least d_max − 1; y lies in the α-widened box"
- [x] T025 [P] Unit test `tests/unit/test_box_to_box.py`: "donors come only from N(i) ∪ {i}; x_d's
  line is at most x_a's line"
- [x] T026 [P] Unit test `tests/unit/test_destroyer.py`: "the better member of each pair is never
  moved; the kicked agent lands outside the niche, or its kick is recorded as failing to leave it"
- [x] T027 [P] Unit test `tests/unit/test_virtuoso.py`: "the poll stops at the first improvement; the
  expand and contract rules hold; the mesh stays within [h_min, h_max]"
- [x] T028 [P] Unit test `tests/unit/test_finisher.py`: "jumps are centred on the ball, not on the
  incumbent; a restart never reuses the last restart index when K > 1"
- [x] T029 [P] Implement `src/tfo/archetypes/sweeper_keeper.py` (mechanism-interface.md §2.1,
...
- [x] T030 [P] Implement `src/tfo/archetypes/zonal_centre_back.py` (§2.2, FR-006): `y ~ U(stratum_k)`
  against the current Latin-hypercube stratification (A9), redrawn at fixture boundaries (makes T022
  pass)
- [x] T031 [P] Implement `src/tfo/archetypes/overlapping_wing_back.py` (§2.3, FR-007):
  quasi-opposite step about the squad centroid with probability Jr, keep-better rule (makes T023
  pass)
- [x] T032 [P] Implement `src/tfo/archetypes/deep_lying_playmaker.py` (§2.4, FR-008): BLX-α
  recombination with a partner at graph distance ≥ d_max − 1, the declared long-range exception to
  FR-002 (makes T024 pass)
- [x] T033 [P] Implement `src/tfo/archetypes/box_to_box.py` (§2.5, FR-009): differential move
  `v = x_nbest(i) + F·(x_d − x_a)` plus binomial crossover (makes T025 pass)
- [x] T034 [P] Implement `src/tfo/archetypes/destroyer.py` (§2.6, FR-010): niche-pair detection and
  unconditional differential kick of the worse member of each pair (makes T026 pass)
- [x] T035 [P] Implement `src/tfo/archetypes/virtuoso.py` (§2.7, FR-011): coordinate-wise compass
  poll with per-agent mesh expand-on-success/contract-on-failure (makes T027 pass)
- [x] T036 [P] Implement `src/tfo/archetypes/finisher.py` (§2.8, FR-012): Lévy(β) jump centred on the
...
- [x] T037 [P] Unit test `tests/unit/test_formation.py`: "the three shapes exist for n; the ratio is
...
- [x] T038 [P] Unit test `tests/unit/test_rotation.py`: "only better-deeper/worse-upfield pairs swap;
  the role-sheet composition is preserved"
- [x] T039 [P] Unit test `tests/unit/test_possession.py`: "the τ acceptance rule holds; the
  neutral-pass cap stops plateau loops; receivers are always neighbours of the carrier"
- [x] T040 [P] Unit test `tests/unit/test_pressing.py`: "the cap on |P| holds; with ρ = 0 no agent
  presses"
- [x] T041 [P] Unit test `tests/unit/test_counter_attack.py`: "the burst's step sizes decay
...
- [x] T042 [P] Unit test `tests/unit/test_set_pieces.py`: "the schedule and the rotation order are
  fixed; on a 1-D quadratic, the free kick hits the vertex exactly"
- [x] T043 [P] Unit test `tests/unit/test_offside.py` (property-based): "the repaired point lies in
  ...
  `ε(T_c) = 0`
- [x] T044 [P] Unit test `tests/unit/test_substitutions.py`: "the cap is honoured; both trigger
  conditions are required; the tabu entry is written"
- [x] T045 [P] Unit test `tests/unit/test_fatigue.py`: "drain is monotone in distance; recovery
  shrinks with t; substitutes enter with s = 1"
- [x] T046 [P] Unit test `tests/unit/test_var_review.py`: "the rollback costs 0 evals; aspiration
  keeps a move that set a new best; the incumbent survives any rollback"
- [x] T047 [P] Unit test `tests/unit/test_manager.py`: "an oscillating input cannot cause state
  changes on consecutive iterations; HIGH_PRESS is never entered before t_press; CHASING is present
  only in TFO"
- [x] T048 [P] Implement `src/tfo/tactics/formation.py` (§2.9, FR-013, FR-002): lattice shapes,
  toroidal von Neumann neighbour tables, graph distances, fully-connected neutral default (makes
  T037 pass)
- [x] T049 Implement `src/tfo/tactics/rotation.py` (§2.10, FR-022): deep-to-upfield row swap at
  fixture boundaries, no wrap across the torus (makes T038 pass; depends on T048 for slot/row
  semantics)
- [x] T050 [P] Implement `src/tfo/tactics/possession.py` (§2.11, FR-014): pass chain with
  τ-threshold retention, neutral-streak cap, R_loss reset (makes T039 pass)
- [x] T051 [P] Implement `src/tfo/tactics/pressing.py` (§2.12, FR-015): distance-gated pressing set
  capped at ⌊0.5·n_out⌋, shrinking-encircling step (makes T040 pass)
- [x] T052 [P] Implement `src/tfo/tactics/counter_attack.py` (§2.13, FR-016): event trigger
  `f(y) < f_b − Δ_mat·max(|f_b|, 1e-12)` and `‖y − x_b‖ > d_trans`, ball relocation, geometrically
  decaying burst (makes T041 pass)
- [x] T053 [P] Implement `src/tfo/tactics/set_pieces.py` (§2.14, FR-017): fixed corner/free-kick
  rotation on a fixed schedule, never adapted (makes T042 pass)
- [x] T054 [P] Implement `src/tfo/tactics/offside.py` (§2.15, FR-018): bound repair
  ...
  (makes T043 pass)
- [x] T055 [P] Implement `src/tfo/tactics/substitutions.py` (§2.16, FR-019): stagnation+stamina
  trigger, per-fixture cap, tabu-register write on the outgoing position (makes T044 pass)
- [x] T056 [P] Implement `src/tfo/tactics/fatigue.py` (§2.17, FR-020): distance-proportional drain,
  fixture-boundary recovery that shrinks over the run, full-stamina substitutes (makes T045 pass)
- [x] T057 [P] Implement `src/tfo/tactics/var_review.py` (§2.18, FR-021): periodic audit against
  tabu/collision/feasibility checks, zero-cost rollback with aspiration (makes T046 pass)
- [x] T058 Implement `src/tfo/manager.py` (§2.19, FR-023–FR-026): the four-state machine (BUILD_UP,
...
- [x] T059 [P] Integration test `tests/integration/test_engine_order.py`: the 9-step iteration order
  of mechanism-interface.md §3 is deterministic and pinned
- [x] T060 [P] Unit test `tests/unit/test_trace.py`: TacticalSegment run-length encoding is lossless
  and contiguous, MechanismEvalCount sums to `evals_used`
- [x] T061 Implement `src/tfo/engine.py`: the iteration loop of mechanism-interface.md §3, wiring
...
- [x] T062 [P] Implement `src/tfo/trace.py`: `RunTrace` (TacticalSegment, MechanismEvalCount, the
  per-run summaries of A14) (makes T060 pass)
- [x] T063 Implement `src/tfo/api.py` and `src/tfo/__init__.py`: `minimize()` returning `TFOResult`
...
- [x] T064 [P] Unit test `tests/unit/test_api_public_entry.py`: `tfo.minimize` runs standalone with
  only NumPy on the import path, and `TFO_STATIC` has `enable.manager = False`

### 2.5 External evaluation ledger and CRN seeds

- [x] T065 [P] Contract test `tests/contract/test_ledger.py`: ledger invariants L3 "For TFO runs,
...
- [x] T066 Implement `src/tfo_bench/ledger.py`: `CountingObjective` (evaluation-ledger.md §1) — batch
...
- [x] T067 [P] Implement `src/tfo_bench/seeds.py`: CRN seed derivation
...
- [x] T068 [P] Unit test `tests/unit/test_seeds_crn.py`: the same (suite, cell, run) key yields the
  identical seed for every algorithm and every ablation/sensitivity variant

### 2.6 Benchmark problems (4 suites)

- [x] T069 [P] Contract test `tests/contract/test_information_parity.py`: "calling `view.constraints`
  raises `CapabilityError` unless the job's experiment is `epsilon`" (C7)
- [x] T070 Implement `src/tfo_bench/problems/base.py`: `Problem`, `ProblemView` (unit-box decoder
  `x = lb + (ub − lb) ⊙ u`, capability gating for `constraints`) (makes T069 pass; depends on T066)
- [x] T071 [P] Implement `src/tfo_bench/problems/cec2017.py`: cec2017-py backend at official
  numbering F1, F3–F30, D = 30 (research.md R3)
- [x] T072 [P] Implement `src/tfo_bench/problems/cec2022.py`: opfunu backend, classes F12022–F122022,
  D ∈ {10, 20} (research.md R4)
- [x] T073 [P] Implement `src/tfo_bench/problems/engineering.py`: the 7 ported problems returning
...
- [x] T074 [P] Implement `src/tfo_bench/problems/tuning.py`: the 7 tuning functions (Sphere,
...
- [x] T075 [P] Implement `src/tfo_bench/problems/reference/`: the opfunu CEC-2017 cross-check
  wrapper and the official-C CEC-2022 ctypes shim, built only for audit and cell-restoration use
  (research.md R3, R4)
- [x] T076 [P] Golden test `tests/golden/test_engineering_port.py`: fixed random/boundary points
  evaluated on the sibling's read-only `engineering_problems.py` must match the port "to 1e-12
  relative" (research.md R10)
- [x] T077 [P] Golden test `tests/golden/test_tuning_optima.py`: every tuning function passes the
  preflight audit at its known optimum

### 2.7 Baseline algorithm adapters and the optimizer contract

- [x] T078 Contract test `tests/contract/test_optimizer_contract.py`, parametrised over all 9 roster
  ...
  T070)
- [x] T079 [P] Contract test `tests/contract/test_vendor_integrity.py`: the SHA-256 of
  `chess_algorithms_96af96b.py` matches `PROVENANCE.md` (research.md R9)
- [x] T080 Implement `src/tfo_bench/algorithms/base.py`: the `Optimizer` Protocol, `OptimizeResult`,
  and the roster registry (optimizer-interface.md §2)
- [x] T081 Implement `src/tfo_bench/algorithms/tfo_adapter.py`: `TFOAdapter(config)` calling
  `tfo.minimize`, plus the ablation/sensitivity variant constructors (depends on T063, T080)
- [x] T082 [P] Implement `src/tfo_bench/algorithms/sibling.py`: `CAAdapter`, `GAAdapter`,
...
- [x] T083 [P] Implement `src/tfo_bench/algorithms/woa.py`: `WOAAdapter` over
...
- [x] T084 [P] Implement `src/tfo_bench/algorithms/lshade.py`: `LSHADEAdapter` over niapy's L-SHADE,
  `population_size = 18·D`, `Task(max_evals=B)` (depends on T080)
- [x] T085 [P] Implement `src/tfo_bench/algorithms/cmaes.py`: `CMAESAdapter` over `cma.fmin2` as
...
- [x] T086 [P] Contract test `tests/contract/test_results_schema.py`: the 9 validation checks of
  results-schema.md §9
- [x] T087 Implement `src/tfo_bench/records.py`: writers and validators for `runs.csv`, `curves.csv`,
...
- [x] T088 [P] Implement `scripts/validate_results.py`: runs the 9 checks of results-schema.md §9
  against a results directory, exit code 1 on any failure
- [x] T089 Implement `src/tfo_bench/runner.py`: job enumeration (`ExperimentJob` of data-model.md
...
- [x] T090 [P] Implement `src/tfo_bench/manifest.py`: `EnvironmentManifest` (git SHA and dirty flag,
...
- [x] T091 [P] Unit test `tests/unit/test_manifest.py`: "a dirty working tree is refused for
...
- [x] T092 [P] Contract test `tests/contract/test_audit_state_machine.py`: the `CellAudit`
  ...
  non_discriminative"
- [x] T093 Implement `src/tfo_bench/audit.py`: preflight tolerance
...
- [x] T094 [P] Implement `scripts/audit_preflight.py`: runs the preflight stage of `audit.py` for
  each suite and writes `results/audit/cell_audit.csv` with one row per cell
- [x] T095 [P] Unit test `tests/unit/test_audit_pre_audit_retention.py`: "Pre-audit numbers are never
  deleted" — a restored or excluded cell's pre-audit numbers remain readable under
  `results/audit/pre_audit/<suite>/…`
- [x] T096 [P] Implement `src/tfo_bench/stats.py`: the error floor (errors below 1e-8 set to 0),
...
- [x] T097 [P] Unit test `tests/unit/test_holm.py`: the in-house Holm correction is unit-tested
  against a published worked example (research.md R12.7)
- [x] T098 Implement `src/tfo_bench/analyze.py`: the generic `runs.csv` → `CellStats`,
...
- [x] T099 Author `config/protocol.toml` with the full pre-registration content: budgets per suite
...
- [x] T100 [P] Implement `scripts/tune.py`: the coordinate-wise tuning procedure of research.md R13
  ...
  T094)
- [x] T101 Tag `prereg-v1` on `config/protocol.toml` and `tfo-frozen-v1` on `config/tfo_frozen.toml`,
  ...
  T099, T100)
- [x] T102 [P] Implement `scripts/pilot.py`: the throughput pilot measuring per-evaluation cost
...
- [x] T103 [P] Integration test `tests/integration/test_config_freeze_guard.py`:
...
- [x] T104 Integration test `tests/integration/test_switchability.py` (gate G2): for each of the 18
...
- [x] T105 Integration test `tests/integration/test_smoke_pipeline.py`: an end-to-end run on one
...
- [x] T106 [P] Integration test `tests/integration/test_reproducibility.py` (contract C2): two runs
...
- [x] T107 [P] Implement `scripts/compare_runs.py`: reports two result directories as bit-identical
  in `runs.csv` and `curves.csv`, ignoring timing columns
- [x] T108 [P] Implement `scripts/run_suite.py`: the thin CLI shared by every suite run —
...
- [x] T109 [P] [US1] Implement `scripts/make_mechanism_table.py`: renders the operator-family table
...
- [x] T110 [P] [US1] Unit test `tests/unit/test_mechanism_table_complete.py`: "100% of TFO's
...
- [x] T111 [P] [US1] Implement `scripts/audit_overlap.py`: computes the CA overlap audit from
...
- [x] T112 [P] [US1] Contract test `tests/contract/test_identifier_allowlist.py` (Principle VI,
...
- [x] T113 [P] [US1] Write `docs/metaphor_free_description.md`: expands mechanism-interface.md §1–§2
...
- [x] T114 [P] [US1] Implement `scripts/check_metaphor_free.py` and unit test
...
- [x] T115 [P] [US1] Unit test `tests/unit/test_homage_sentence_cap.py`: for each of the 8 archetypes
...
- [x] T116 [US1] Assemble `docs/reviewer_audit_packet.md` bundling the rendered operator-family table
