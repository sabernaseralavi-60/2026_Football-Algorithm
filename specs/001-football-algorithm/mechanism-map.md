...
| Regista (deep-lying playmaker) | Long diagonal pass: recombines with a partner at maximal graph distance in the formation, bypassing local topology | BLX-α crossover over small-world long-range links (Eshelman & Schaffer 1993; Watts & Strogatz 1998) |
...
| Finisher (explosive centre-forward) | Mostly quiet, occasionally explosive: heavy-tailed jumps around the ball; after repeated misses it restarts from a different Sweeper-Keeper archive elite | Lévy-flight (heavy-tailed) mutation with elite restart (Lee & Yao 2004; Mantegna 1994) |
| **Team-level tactics** | | |
| Formation (4-4-2, 4-3-3, back five …) | The squad sits on a lines × lanes pitch lattice, and each agent interacts only with its von Neumann teammates; the formation is the lattice's shape (compact ↔ stretched) | Cellular (structured) population with dynamic grid-shape topology (Alba & Dorronsoro 2005; Kennedy & Mendes 2002) |
| Positional rotation ("total football") | At fixture boundaries, a better agent in a deeper line swaps slots, and therefore archetype, with a worse upfield neighbour | Rank-based role reassignment restricted to graph neighbours |
| The ball and possession | A single focal point is passed between formation neighbours by short drift steps; a pass is *retained* only if fitness does not regress by more than τ. Closest prior art is the Tiki-taka Algorithm's ball matrix (short passes to a nearby player, random chance of loss — see `related-work.md`); TFO also decouples the ball from the incumbent (unlike Soccer Game Optimization, where the ball carrier *is* the best solution so far) but replaces TTA's random loss with the τ-threshold retention rule | Threshold accepting (Dueck & Scheuer 1990); τ = 0 gives non-regression with neutral drift |
...
| Substitutions | An agent whose personal best has stalled for *W* iterations and whose stamina is low is replaced by a fresh uniformly random agent, at most five per fixture; the outgoing position is marked tabu | Stagnation-triggered random immigrants, capped (Grefenstette 1992) |
